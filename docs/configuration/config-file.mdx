...
| `idle_timeout` | duration string (e.g. `"30m"`, `"1h"`) | disabled | Optional idle shutdown for manual `monocle serve` after the last client disconnects (plus a 60s grace window). Overridden by the `--idle-timeout` flag; use `"0s"` in config or a negative value to disable. |
...
## WakaTime and Wakapi tracking

WakaTime-compatible review tracking is controlled by an environment variable, not by the JSON config file. Set `MONOCLE_WAKATIME_ENABLED=1` when launching monocle:

```bash
MONOCLE_WAKATIME_ENABLED=1 monocle
```

monocle invokes `wakatime-cli` and lets it use your existing WakaTime or Wakapi setup. Review activity is reported as `code reviewing` under the `Monocle` editor while the TUI is recently active. monocle handles review heartbeat rate limiting itself and disables the CLI's own limiter for those events.
