...
| `Ctrl+d` / `Ctrl+u` or `PageDown` / `PageUp` | Scroll diff half page (any pane) |
...
| `r` / `Space` | Toggle the current file as reviewed (`Space` in the sidebar; auto-advances to the next unreviewed file) |
...
The comment editor uses standard Emacs-style shortcuts. Terminal paste inserts text in the comment editor, submit modal, and command mode.
...
| `Tab` | Cycle comment type (issue, question, suggestion, note) |
