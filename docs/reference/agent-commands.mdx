...
When registered in **MCP tools mode**, agents call these operations as MCP tools (`review_status`, `get_feedback`, `send_artifact`, `add_files`, `reply_to_thread`) instead of CLI commands. The behavior is identical — the MCP server connects to the same Unix socket internally.

When registered in **skills mode**, agents invoke the corresponding CLI commands below.

<Note>
  Agent commands first try the socket for the current directory. If the agent is in a non-git parent folder and exactly one running monocle session exists in a nested repo, commands use that nested session automatically. If several nested sessions match, the command lists them and asks you to rerun with `--workdir` (`-C`) for the repo you want. You can also set `MONOCLE_SOCKET` or `MONOCLE_WORKDIR`. Press `I` in the monocle TUI to see the active socket path.
</Note>
...
Each submitted comment includes a `Thread ID`. Use `monocle review reply --comment-id <thread-id>` or the `reply_to_thread` MCP tool to answer questions or explain what changed in that same open thread. Agents cannot resolve threads; only the reviewer can resolve them in the TUI.
...
## `monocle review reply`

Reply to an unresolved reviewer comment thread.

```bash
monocle review reply --comment-id ID [--body TEXT] [--json] [--workdir PATH] [--socket PATH]
```

When `--body` is omitted, the command reads the reply body from stdin. Use the `Thread ID` shown in feedback as the `--comment-id` value.

### Flags

| Flag | Short | Description |
|------|-------|-------------|
| `--comment-id ID` | | **(Required)** Thread/comment ID from the submitted feedback. |
| `--body TEXT` | | Reply body. If omitted, reads stdin. |
| `--json` | | Output the result as structured JSON. |
| `--workdir PATH` | `-C` | Override the working directory. Also reads from `MONOCLE_WORKDIR`. |
| `--socket PATH` | | Override the Unix socket path. Also reads from `MONOCLE_SOCKET`. |
...
```bash
monocle review reply --comment-id 018f... --body "Fixed by reusing the existing validator."
```

```bash
printf '%s\n' "I left this unchanged because the generated file is intentionally checked in." \
  | monocle review reply --comment-id 018f...
```

### MCP tool equivalent

In MCP tools mode, call the `reply_to_thread` tool:

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `comment_id` | string | Yes | Thread/comment ID from the submitted feedback. |
| `body` | string | Yes | Reply text to append to the thread. |
