...
Monocle detects the Git repository root from the current working directory and opens the TUI. By default it resumes the most recent session for the repository, or starts one if none exists. Use `--new` to force a fresh session.
...
| `--new` | `-n` | Start a new session for this repository. |
...
`--new`, `--continue`, `--resume`, and `--session` are mutually exclusive — only one may be used at a time.
...
| Agent | Skills directory | Commands | MCP config |
|-------|------------------|----------|------------|
...
Run a headless monocle engine for the current repository. This process owns the socket, the SQLite database, and the review session; the `monocle` TUI and agent commands connect as thin clients over the socket.

```bash
monocle serve [--workdir PATH] [--socket PATH] [--idle-timeout DURATION]
```

You don't normally run `monocle serve` yourself. Launching `monocle` auto-spawns one for the current repo if none is running, and the TUI stops that owned server when it exits. Run `monocle serve` manually when you want the engine up without a UI, or when you want it to keep running after the TUI exits.
...
| `--idle-timeout DURATION` | | Exit after this idle interval past the 60s grace window (for example, `30m`, `1h`). Disabled by default; use a positive duration to opt in or a negative duration to disable a configured timeout. |

### Idle shutdown

An auto-spawned `monocle serve` is owned by the TUI that created it and stops when that TUI exits. A manually started `monocle serve` keeps running until you run `monocle stop` or send it a signal. If you set `--idle-timeout` or `idle_timeout`, the manual server exits after every connected client has been gone for 60 seconds plus that duration.
...
Start the built-in MCP server over stdio. This exposes monocle's review operations (`review_status`, `get_feedback`, `send_artifact`, `add_files`, `reply_to_thread`) as MCP tools that any stdio-compatible agent can call.
