  ...
  <Accordion title="No feedback delivered to the agent">
    <Steps>
      <Step title="Confirm monocle is running in the right directory">
        monocle must be running for the Git repository you want reviewed. Agent commands can discover a unique nested running repo from a parent workspace, but they will ask you to choose if several nested repos are running. Check the title bar or press `I` to see the working directory monocle is using.
      </Step>
