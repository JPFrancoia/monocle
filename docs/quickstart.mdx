...
<Steps>
  <Step title="Install monocle">
    Follow the [installation guide](/installation) to build monocle locally.
  </Step>
  ...
  <Step title="Register monocle with your agent">
    ...
    The interactive picker lets you select which agents to register. Pass a named agent — `claude`, `opencode`, `codex`, `gemini`, or `all` — to skip the picker. Use `--global` to write to your user-level config directory instead of the current project.

    This installs skills into the agent-specific directory for your project. For Claude Code, it also writes the MCP channel config to `.mcp.json` for push notifications. For OpenCode, it also installs slash commands into `.opencode/commands/`.
  ...
  <Step title="Review your agent's code">
    ...
    - `c` — add a comment at the cursor (tag it as an issue, question, suggestion, or note)
    - `v` — start a visual selection for multi-line comments
    - Mouse drag — select rendered text and copy it on release
    - `S` — submit your review

    When you submit, monocle formats your comments and queues the review for delivery. The agent receives your feedback, addresses it, and can reply in each current-round thread. You see the updated diffs, review again without prior-round comments cluttering the view, resolve satisfied threads, and iterate.
