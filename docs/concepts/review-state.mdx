...
Each round builds on the last, and monocle remembers what the code looked like when you last reviewed it, so you don't have to re-examine everything from scratch. Comments from prior rounds stay in submission history, but the active diff view only shows comments from the current round.
