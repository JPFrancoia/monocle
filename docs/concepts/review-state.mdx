...
Each round builds on the last, and monocle remembers what the code looked like when you last reviewed it, so you don't have to re-examine everything from scratch. Comments from every round remain visible in the session so you can follow threads and agent replies until you resolve or delete them.

## Reviewed state

Every file in the sidebar has a reviewed state: either reviewed (green checkmark) or unreviewed (empty circle). You toggle this with `r` or with `Space` when the sidebar is focused, and filter the sidebar with `/` to show only unreviewed or reviewed files.

When you press `r` or `Space` in the sidebar, monocle records the file content you reviewed. If that file changes again, monocle automatically clears the reviewed mark so it shows up in the unreviewed filter again.

When you submit a review, monocle also captures a full snapshot for the round. That snapshot lets monocle compare the next set of agent changes against the state you submitted.

## Snapshots and change detection

When you submit a review with **Request Changes**, monocle creates a **snapshot** — a record of every file's content at that moment. On the next round, monocle compares each file against either your latest manual reviewed mark or the snapshot:
...
| `"manual"` | Only files you explicitly toggled with `r` or `Space` in the sidebar are marked as reviewed. |
...
You read each diff. `auth.go` looks good, so you press `r` or `Space` in the
sidebar, to mark it reviewed. You notice a bug in `router.go` and leave a
comment. You mark it reviewed too, so you can track that you've seen it:
