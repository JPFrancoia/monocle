...
<Steps>
  ...
  <Step title="Submit your review">
    Press `S` to open the submit modal. Your review is formatted and queued for delivery. The agent picks it up, addresses each comment, and can reply in the original thread.
  </Step>
  <Step title="Iterate">
...
| **Question** | Things you need the agent to answer before you decide |
| **Suggestion** | Improvements that would be nice but aren't blockers |
| **Note** | Observations or context that should stay with the review |
...
Mouse drag selects rendered terminal text and copies it on release. Use keyboard visual selection when you want to attach a multi-line comment.

<Note>
  You can mark a comment as resolved with `x`. Threads stay visible in the session until you resolve or delete them, even after the agent picks up feedback and the review round advances. Submitted reviews include unresolved comments from the current round; older threads remain visible for context and replies. Agents can reply to open threads, but only you can resolve them. To delete a comment entirely, press `d` while on a comment line.
</Note>

### Comment expansion

Comments expand automatically when you hover over them, showing the full comment text with syntax highlighting. Agent replies appear inside the expanded thread block. Press `Space` on a comment line to manually toggle it expanded or collapsed. You can disable auto-expansion or adjust the delay in your [config file](/configuration/config-file) with the `comment_expand` and `comment_expand_delay` settings.
...
- Each comment includes a thread ID. Agents use that ID to answer questions or explain changes in the same thread.
...
- **Paste**: Terminal paste inserts text into the submit modal. Paste also works in the comment editor and command mode.
