...
<Note>
  You can mark a comment as resolved with `x`. Resolved threads stay visible for context during the current round, but they are excluded from submitted reviews. Prior-round comments stay in submission history and do not clutter the active diff. Agents can reply to open current-round threads, but only you can resolve them. To delete a comment entirely, press `d` while on a comment line.
</Note>
