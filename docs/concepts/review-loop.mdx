...
<Note>
  You can mark a comment as resolved with `x`. Resolved threads stay visible for context, but they are excluded from submitted reviews. Agents can reply to open threads, but only you can resolve them. To delete a comment entirely, press `d` while on a comment line.
</Note>
