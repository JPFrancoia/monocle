...
- **Skills mode** installs `SKILL.md` instruction files ([agentskills.io](https://agentskills.io) format) that tell the agent which `monocle review` CLI commands to run. OpenCode also gets matching slash commands. This is the default for all other agents.

You can override the default with `--integration-mode` when registering. See [agent setup](/guides/agent-setup) for details.

## Available operations

These operations are available through MCP tools, skills, or direct `monocle review` CLI commands:
...
| Reply to thread | `reply_to_thread` | — | Answer a reviewer comment thread without resolving it |
...
Submitted comments include a `Thread ID`. Agents should use that ID with `reply_to_thread` to answer questions or explain what changed after addressing feedback. Resolving threads is reviewer-only.
...
Adds an agent reply to an unresolved reviewer comment thread. The thread stays open until you resolve it in the TUI.

## Registration

`monocle register` writes the appropriate config for your agent. In MCP tools mode, it configures an MCP server entry and installs slash commands. In skills mode, it writes skill files to the agent's skill directory; OpenCode also gets slash commands.
...
```bash OpenCode
# Default: skills mode — installs to .opencode/skills/ and .opencode/commands/
...
- When feedback includes a Thread ID, answer questions or explain changes in that thread. Do not try to resolve threads.
