...
The TUI title bar shows the active repository name, current branch, and selected file path, so you can confirm which workspace and file you are reviewing at a glance.

## Starting a session

```bash
monocle                        # resume the latest session, or start one if none exists
monocle --new                  # start a new session
...
Use `--new` when you want a clean review context for the same repository. This is useful if you use a Git commit as your approval point and want the next review to start without the previous session's comments and reviewed marks.
...
Mark a file as reviewed by pressing `r` or `Space` when it's selected in the sidebar. monocle automatically advances to the next unreviewed file.
